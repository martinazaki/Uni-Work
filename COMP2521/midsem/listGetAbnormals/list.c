...
void checkValidity(List l) {
	...
	if (forwardLength != backwardLength) {
		printf("Error: the number of nodes in the forwards direction "
		       "does not equal the number of nodes in the backwards "
		       "direction\n");
	// Check 2)
	} else if (forwardLength != l->size) {
		printf("Error: the number of nodes in the list is not equal to "
		       "the list's size field\n");
	// Check 3)
	} else {
		for (int i = 0; i < forwardLength; i++) {
			if (forwardAddrs[i] != backwardAddrs[forwardLength - i - 1]) {
				printf("Error: the sequence of addresses of nodes in "
				       "the forwards direction does not match the sequence "
				       "of addresses of nodes in the backwards direction\n");
				break;
			}
		}
	}
