...
main:
    ...
    # array[x][y] = array + (24*x+y)*4
    
    move $t3, $t0       # t3 = x
    li   $t4, 24        # t4 = 24 
    mul  $t3, $t3, $t4  # t3 = x*24
    add  $t3, $t3, $t1  # t3 = x*24+y
    li   $t4, 4         # t4 = 4
    mul  $t3, $t3, $t4  # (x*24+y)*4 offset
    la   $t4, array     # t4 = address of array
    add  $t3, $t3, $t4  # t3 = address of array[x][y]
    lw   $t3, ($t3)     # t4 = value of array[x][y]    

    move $a0, $t3         # printf("%d", array[x][y])
    li   $v0, 1
    syscall
